...
export const updateEventStats = functions.https.onCall(async () => {
  ...
  const signups = signupsSnapshot.docs.map((doc) =>
    Object.assign(doc.data(), { event: doc.ref.parent.parent!.id })
  );
  ...
  const eventInfos = events.map((event) => {
    ...
    const waitListRegistrations = eventSignups.filter((s) => s.isWaitlist)
      .length;
    const payedRegistrations = eventSignups.filter((s) => s.hasPayed).length;
    const attendedRegistrations = eventSignups.filter((s) => s.hasAttended)
      .length;
  ...
  const stats = {
    ...
    registrationNum: eventInfos.reduce(
      (acc, curr) => acc + curr.totalRegistrations,
      0
    ),
    attendedNum: eventInfos.reduce(
      (acc, curr) => acc + curr.attendedRegistrations,
      0
    ),
    waitListNum: eventInfos.reduce(
      (acc, curr) => acc + curr.waitListRegistrations,
      0
    ),
    cost: eventInfos.reduce((acc, curr) => acc + curr.cost, 0),
    collected: eventInfos.reduce(
      (acc, curr) => acc + curr.price * curr.payedRegistrations,
      0
    ),
    payed: {
      eventNum: eventInfos.filter((e) => e.payed).length,
      registrationNum: eventInfos
        .filter((e) => e.payed)
        .reduce((acc, curr) => acc + curr.totalRegistrations, 0),
      attendedNum: eventInfos
        .filter((e) => e.payed)
        .reduce((acc, curr) => acc + curr.attendedRegistrations, 0),
      waitListNum: eventInfos
        .filter((e) => e.payed)
        .reduce((acc, curr) => acc + curr.waitListRegistrations, 0),
      payedNum: eventInfos
        .filter((e) => e.payed)
        .reduce((acc, curr) => acc + curr.payedRegistrations, 0),
      cost: eventInfos
        .filter((e) => e.payed)
        .reduce((acc, curr) => acc + curr.cost, 0),
      collected: eventInfos
        .filter((e) => e.payed)
        .reduce((acc, curr) => acc + curr.price * curr.payedRegistrations, 0),
    },
  };
...
export const updateUserStats = functions.https.onCall(async (call, context) => {
  ...
  const userInfos = users.map((user) => {
    ...
    const attendedRegistrations = eventSignups.filter((s) => s.hasAttended)
      .length;
    const tutoredEvents = events.filter(
      (event) => event.tutorSignups && event.tutorSignups.includes(user.id)
    ).length;
    const moneySpent = eventSignups
      .filter((s) => s.hasPayed)
      .reduce(
        (acc, curr) => acc + events.find((e) => e.id === curr.event)!.price,
        0
      );
  ...
  const stats = {
    ...
    userNumAttended: userInfos.filter((u) => u.attendedRegistrations > 0)
      .length,
    userNumRegistered: userInfos.filter((u) => u.totalRegistrations > 0).length,
    registrationNum: userInfos.reduce(
      (acc, curr) => acc + curr.totalRegistrations,
      0
    ),
    attendedNum: userInfos.reduce(
      (acc, curr) => acc + curr.attendedRegistrations,
      0
    ),
    waitListNum: userInfos.reduce(
      (acc, curr) => acc + curr.waitListRegistrations,
      0
    ),
