...
describe('Event Participant Registration', () => {
  ...
  context('when user 1 is logged in', () => {
    beforeEach(() => {
    ...
    it('should inform the user of a missing payment', () => {
      cy.get('[data-testid="event-signup-section"]').contains(
        'You can register for this event'
      );
      ...
      cy.origin('https://checkout.stripe.com', { args: cards }, ({ visa }) => {
        ...
        cy.get(
          'a[title="Erasmus Student Network TUMi (ESN TUMi) e.V."]'
        ).click();
      });
      cy.get('[data-testid="event-signup-section"]', {
        timeout: 15000,
      ...
      cy.get('[data-testid="event-signup-section"]', {
        timeout: 15000,
      }).contains('Your payment is not complete yet');
    });
  });
  context('when user 2 is logged in', () => {
    beforeEach(() => {
      cy.loginByAuth0Api(users.regularUser2);
      cy.visit(`/events/${events.paidEvent.id}`);
      cy.waitForLoad();
    });
